...
Select the column of `carrier`, `flight`, `year`, `month`, `day` and use `make_date()` to create a date. Name the date into `departure`. Then using `arranging()` to re-order the column of `departure` by a descending order. 
...
Try to understand the command of creating the new function called `make_arrival` and complete the command.  Then we are going to analyze the arrival information of the flights with destination to `Atlanta (“ATL”)`. Create a datetime for the arrival time of these flight and name it into `arrival_time`. Select the column of `carrier`, `flight` and `arrival_time`. At last, try to use `ggplot()` and `geom_freqpoly()` to make a plot of `arrival_time`(use binwidth = 86400). Colored the flight by `carrier` so that people could easily understand the difference of the number of flights from New York to Atlanta among `carriers` . Refer to the graph showing below, try your best to answer this question and get the same outcome as the plot below.

```{r plot16-2-4_Challenging_MS}
