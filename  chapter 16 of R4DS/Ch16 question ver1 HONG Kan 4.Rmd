...
```{r 16-2-4EASYHK, exercise=TRUE, exercise.lines = 5}
...
### 16.2.4 Intermediate HK

Exercise 1 HK

Use the as_date function to show January 25, 2022:

```{r 16-2-4IntermediaHK, exercise=TRUE, exercise.eval=TRUE}

```

```{r 16-2-4IntermediaHK-hint}
as_date(365 *  +  )
```

```{r 16-2-4IntermediaHK-solution}
as_date(365 *52+ 37)
```

```{r 16-2-4IntermediaHK-check}
...
```{r 16-2-4Intermediate2HK, exercise=TRUE, exercise.eval=TRUE}

```

```{r 16-2-4Intermediate2HK-hint}
as_datetime(60 * 60 * )
```

```{r 16-2-4Intermediate2HK-solution}
as_datetime(60 * 60 * 24* 25 - 60 * 60)
```



### 16.2.4 Challenging HK
...
```{r 16-2-4ChallengingHK, exercise=TRUE, exercise.eval=TRUE}
make_datetime_100 <- function(year, month, day, time) {
  make_datetime(year, month, day, time %/% 100, time %% 100)
}
...
```{r 16-2-4ChallengingHK-solution}
make_datetime_100 <- function(year, month, day, time) {
  make_datetime(year, month, day, time %/% 100, time %% 100)
}
...
flights_dt %>% 
  filter(dep_time < ymd(20130105)) %>% 
  ggplot(aes(dep_time)) + 
  geom_freqpoly(binwidth = 86400)
```

```{r 16-2-4ChallengingHK-check}
gradethis::grade_this_code()
```
