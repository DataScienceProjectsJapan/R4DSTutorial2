...
```{r 16-2-1EasyLW-solution}
ymd(20170131, tz = "UTC")
```

```{r 16-2-1EasyLW-check}
gradethis::grade_this_code()
```

### 16.2.1 Easy LW

Use the lubridate function to parse the date “January 26, 2022”. Choose from ymd(), mdy() and dmy():
...
```{r 16-2-1Easy2LW-solution}
mdy("January 26th, 2022")
```

```{r 16-2-1Easy2LW-check}
gradethis::grade_this_code()
```

### 16.2.2 Intermediate LW

Create a date/time from the flights data by using make_date() function, select columns of “talinum”, columns end with “delay”, “year”, “month”, “day”. Put the date into “departure” with mutate() function:
...
flights %>% 
     select(tailnum, ends_with("delay"), year, month, day) %>% 
...
Create a new function “make_datetime_new” to break the times into hours and minutes. Use modulus arithmetic to pull out the hour and minute components from the “sched_dep_time” column. Visualize the distribution of scheduled departure time (sched_dep_time) within one week (from January 1, 2013 to January 7, 2013), with a binwidth of 1 hour.
