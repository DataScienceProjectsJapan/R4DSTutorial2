...
From the `flightsDateTime` dataset created above (you need to copy the code created above to this code block), plot a graph to find out which timeframe has the largest number of departure flight on January 3rd, 2013. Use `geom_freqpoly` with `binwith` of 600 (10 minutes). (you can use `?geom_freqpoly` to find out how to use the function)

```{r 16-2-2IntermediateTD2, exercise=TRUE, exercise.eval=FALSE}
flightsDateTime <- #insert the code in the previous section here
```
...
```{r 16-2-2IntermediateTD2-solution, warning=FALSE}
flightsDateTime <- mutate(flights, depart_date = make_date(year, month, day))
flightsDateTime %>% 
...
Write a function named numToDate that takes an integer representing the number of days from the Unix Epoch Time, and print out and return the date represented by that number.
